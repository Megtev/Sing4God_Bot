...
def get_list_song(text_fp):
	...
	template = re.compile(r"\b[\w'’]+")

	for x in range(len(text_fp[0])):
		if x == 0:
			text_fp[0][0] = text_fp[0][0].rstrip()
			continue
		text_fp[0][x] = ' '.join((text_fp[0][x][word.start():word.end()].lower() for word in template.finditer(text_fp[0][x])))
...
def push(path, path_pkl, token='b9f7f14cbeaf0c7ee2710265b99c9baafae2cdf7baea2be8803020ba34e7'):
	try:
